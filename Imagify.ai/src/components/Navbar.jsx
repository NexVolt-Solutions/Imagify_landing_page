  ...
  return (
    ...
    >
      <nav className="flex items-center justify-between h-20">
        {/* Logo */}
        <Link to="/" className="relative inline-flex items-center" aria-label="Imagify AI - Home">
          <img
            src={assets.logo.src}
            alt="Imagify AI Logo - AI Image Generation Platform"
            className="w-35 md:ml-4 md:w-55"
            width="140"
            height="40"
          />
        </Link>
        ...
        {/* Desktop Nav (LG+) */}
        <ul className="hidden lg:flex gap-6 text-white text-sm" role="list">
          {navLinks.map((link) => (
            <li key={link.name}>
              <a
                href={link.hash}
              ...
              >
                ...
                {/* Yellow underline (LG screens only) */}
                <span
                  className="
        ...
        <div className="flex items-center gap-4">
          ...
          <a
            ...
            className="hidden lg:flex px-6 py-2 rounded-xl text-white font-semibold hover:scale-105 transition"
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <>
            ...
            <motion.aside
              id="mobile-menu"
              className="fixed top-0 right-0 w-64 h-full bg-black z-50 lg:hidden"
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
              role="dialog"
            ...
            >
              <nav className="p-6 flex flex-col bg-black gap-6">
                ...
                <ul role="list" className="flex flex-col gap-2">
                  {navLinks
                    .filter(link => link.name !== "Why Imagify AI" && link.name !== "FAQs")
                    .map(link => (
                      <li key={link.name}>
                        <a
                          href={link.hash}
                          ...
                          className="text-white text-lg pl-6 py-2 block"
                          aria-label={link.ariaLabel}
                        >
                          {link.name}
                        </a>
                ...
                <a
                  ...
                  style={{ background: "linear-gradient(90deg, #BA8B02, #181818)", fontWeight: 600 }}
                  aria-label="Contact Imagify AI support team"
                >
                  Contact Us
                </a>
